...
- A *tile map* is a 2D array of sprites intended for creating maps. They are carefully organized so that they can be indexed one-by-one.
+
image:islandfun_tiles.png[]
- A *level map* references the tile map to build a level.
+
